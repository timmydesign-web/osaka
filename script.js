...
function slideModalDay(direction) {
    ...
    setTimeout(() => {
        ...
        if(newDayNum === 1 || newDayNum === 8) {
            setTimeout(updateFlightStatus, 600);
        }
    }, 200); 
}
...
async function addExpense() {
    ...
    const newExpense = { 
        ...
        amount: amount, 
        desc: descInput.trim(), 
        cat: document.querySelector('input[name="exp-cat"]:checked').value, 
...
function renderCategorySummary() {
    ...
    expenses.forEach(exp => { 
        ...
        if(totals[cat] !== undefined) {
            totals[cat] += parseInt(exp.amount); 
        ...
        listContainer.innerHTML += `
            <div class="exp-item" style="animation-delay: ${itemIndex * 0.05}s;">
                <div class="exp-item-left">
                    <div class="exp-avatar ${exp.payer === 'Timmy' ? 'color-timmy' : 'color-jj'}">${exp.payer === 'Timmy' ? '👦🏻' : '👧🏻'}</div>
                    <div class="exp-info">
                        <span class="exp-desc">${exp.cat ? `<span class="exp-cat-tag">${exp.cat}</span>` : ""}${exp.desc}</span>
